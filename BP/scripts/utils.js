...
/**
 * Returns a number limited to a range
 * - Example: if `x = 5` and `min = 10` then 10 will be returned
 * - if `min = 3` then 5 will be returned

 * @param {Number} x Set number
 * @param {Number} min Minimum Value
 * @param {Number} max Maximum Value
 * @returns {Number} the number within the range
 */
export function limitTo(x,min,max) {
